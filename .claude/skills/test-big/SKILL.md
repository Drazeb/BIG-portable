...
À quelle phase voulez-vous reprendre ?
  ...
  3B-7e — Génération des visuels (prompts MJ/Recraft, images générées, analyse visuelle — skill /visual-brief en session séparée)
...
| `3B-7e` | `{brand}-visual-brief.md`, `{brand}-visual-c*-*.*` (images générées MJ/Recraft via skill /visual-brief séparé — APRÈS le pitch) |
...
| `3B-7e` | `### Étape 3B-7e — Génération visuels MJ/Recraft (skill séparé `/visual-brief`)` dans l'Étape 3B (= ancien `3B-6` = ancien `Étape 3C`) |
...
| `9` | `## ÉTAPE FINALE — Packaging des livrables` (création du dossier `{brand}-identity-{slug}/` avec fichiers renommés ; inclut le sous-dossier `brand-book/` si Phase 8 exécutée) |

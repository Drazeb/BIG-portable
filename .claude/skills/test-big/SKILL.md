...
| `3B-2` | `{brand}-buckets.md` + `{brand}-vm-palette-directive.md` (intermédiaires dérivés), `{brand}-palette-c{1,2,3}-{a,b,c,d,e}.md` (15 variantes a→e — divergence séquentielle dégressive), `{brand}-palette-c{1,2,3}.md` (3 canoniques copiés après choix user), `{brand}-palette-comparison.html` (planche mockups 3 concepts × 5 variantes) |

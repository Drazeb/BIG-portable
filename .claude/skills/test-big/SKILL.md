...
À quelle phase voulez-vous reprendre ?
  ...
  3B-2 — Palettes (buckets d'aptitude ; 5 palettes a→e par concept, divergence dégressive mécanique, 3 gates, planche comparative, choix utilisateur, mini-check aversions couleur)
  ...
  3B-7b — Spécimens stylisés (3 sub-agents parallèles : 3 variantes A/B/C du concept retenu — mode mono D65)
...
| `3B-3` | + `{brand}-concepts-narratifs.md` + `{brand}-chromatic-gamuts.md` + `{brand}-palette-c*.md` (canoniques) + variantes `{brand}-palette-c*-{a,b,c,d,e}.md` (penseurs typo + designer typo arrivent APRÈS la palette ; ils consomment chromatic-gamuts, pas la palette) |
...
| `3B-2` | `{brand}-buckets.md` + `{brand}-vm-palette-directive.md` (intermédiaires dérivés), `{brand}-palette-c1-{a,b,c,d,e}.md` (5 variantes a→e — divergence séquentielle dégressive, mode mono D65), `{brand}-palette-c1.md` (1 canonique copié après choix user), `{brand}-palette-comparison.html` (planche mockups 5 variantes) |
| `3B-3` | `{brand}-penseur-c1.md` (longlist display 12-15 fonts), `{brand}-penseur-body-c1.md` (longlist body 10 fonts), `{brand}-descriptions-c1.md` (descriptions sans noms), `{brand}-font-backups.md` (top 3 display+body avec backups), `{brand}-font-recap-all.html` (planche unifiée 6 fonts), `font-pool-duo-display-c1-*.png` (planches duos display), `font-pool-duo-body-c1-*.png` (planches duos body), `font-pool-font-selection-c1.png` (planche récap) |
| `3B-4` | `{brand}-specimen-c1.html`, `{brand}-specimen-c1.png` |
| `3B-7a-pre` | `{brand}-style-sectoriel-tags.md` (tableau 34 styles × tag binaire SECTORIEL/NON-SECTORIEL + justification 1 ligne — source de vérité pour Gate 5 INTER-variantes A/B/C intra-concept) |
| `3B-7a` | 3 fichiers `{brand}-style-choice-c1-{a,b,c}.md` (3 sous-vagues séquentielles A→B→C : matching libre, divergence vs A, registre ≠ A et B — enchaînement automatique, mode mono D65) |
| `3B-7b` | 3 fichiers `{brand}-style-specimen-c1-{a,b,c}.html` (1 spécimen par variante A/B/C). |
| `3B-7-checkpoint` | Au checkpoint, l'orchestrateur (a) injecte un bandeau informatif (concept + variante + style + palette + fonts) en sticky en haut de chaque spécimen, (b) compose 1 fichier `{brand}-style-scan-matrix-c1.html` (matrice 34 styles × 3 variantes A/B/C avec COMPATIBLE/INCOMPATIBLE + raison au survol + encadré orange sur le style dominant retenu), (c) ouvre les 4 fichiers dans 4 onglets séparés (3 spécimens + 1 matrice). Outputs après choix utilisateur : `{brand}-style-choice-c1.md` (canonique copié depuis la variante choisie) **+ `.concept-pour-3B-7c`** (fichier de marquage simple contenant "1" en mode mono D65 — écrit déterministiquement par l'orchestrateur). |
...
| `3B-7d` | `{brand}-pitch-c1.md` (concept retenu, mode mono D65), `{brand}-pitch.md` (= simple copie de pitch-c1.md, plus d'assemblage multi-concept depuis D65) |
...
| `4` | `{brand}-style-tile-concept-1.html` (1 fichier pour le concept retenu, hero + zone médiane + atmosphere, AVEC placeholder artefact — mode mono D65) |
| `4-val` | (aucun nouveau fichier — validation conversationnelle ; backups `.bak-iter0-pre-correction` créés si itération) |
| `4-art` | (aucun nouveau fichier — modifie in-place `{brand}-style-tile-concept-1.html` en injectant l'artefact à la position du placeholder) |
...
- Démarrage **3B-3** (typo) → idem 3B-2 + palette-c* canoniques + 15 variantes palette-c*-{a,b,c,d,e} + palette-comparison
...
| `3B-2` | `### Étape 3B — Design Dérivé (Pass B)` — commence à la `#### Vague 1 — Palettes par divergence séquentielle dégressive` (pré-calcul buckets + directive ; 5 palettes a→e par concept, divergence mécanique, 3 gates, planche comparative, choix utilisateur) |
...
| `3B-7b` | `#### Étape 3B-7b — Spécimen stylisé (3 sub-agents PARALLÈLES — 3 variantes du concept retenu)` dans l'Étape 3B (puis enchaîne 3B-7-checkpoint) |
